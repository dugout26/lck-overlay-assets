...
(function () {
  ...
  /* 라이엇 피드가 공급하는 최신 시점까지의 지연(초) — 엔드포인트별로 다르고 경기·시점에 따라 변한다.
     너무 최신을 요청하면 400이므로, 실패 시 10초씩 물러나고 성공이 이어지면 다시 당겨 최소 지연을 유지한다.
     (2026-08 실측: 요청 시각이 30초보다 최신이면 400. 허용되는 가장 최신 요청으로 받아도
     실제 데이터는 window ≈ 25초, details ≈ 30초 전 — 이게 라이엇이 주는 물리적 하한이다) */
  var feedLag = { window: 30, details: 30 };
  var feedOk = { window: 0, details: 0 };
  async function fetchNewest(kind, gameId, target, isLive) {
    if (!isLive) return jOr(FEED + "/" + kind + "/" + gameId + "?startingTime=" + iso(floor10(target)));
    for (var tries = 0; tries < 4; tries++) {
      var t = Math.min(target.getTime(), Date.now() - feedLag[kind] * 1000);
      var r = await jOr(FEED + "/" + kind + "/" + gameId + "?startingTime=" + iso(floor10(new Date(t))));
      if (r && r.frames && r.frames.length) {
        /* 연속 성공이 쌓이면 10초 더 당겨본다 — 라이엇이 더 빨리 주는 시간대·경기에서 지연 최소화 */
        if (++feedOk[kind] >= 6 && feedLag[kind] > 30) { feedLag[kind] -= 10; feedOk[kind] = 0; }
        return r;
      }
      feedOk[kind] = 0;
      if (feedLag[kind] >= 90) return null;
      feedLag[kind] += 10;
    }
    return null;
  }
  ...
  async function fetchAndEmit(gameId, gameNumber, gameStart, target, goldHistory, opts, isLive) {
    /* window(골드·KDA 등 실시간 수치)와 details(룬·아이템)는 공급 지연이 달라 각각 최신 시점을 쓴다 */
    var win = await fetchNewest("window", gameId, target, isLive);
    var det = await fetchNewest("details", gameId, target, isLive);
    if (!win || !det) { log("프레임 없음 (204) — 대기"); return; }
    /* 라이브에서는 실제로 받아온 프레임 시각이 표시 기준 (요청 목표보다 최신일 수 있음) */
    if (isLive) target = new Date(win.frames[win.frames.length - 1].rfc460Timestamp);
    ...
    /* ── 라이브: 매치 단위 추적 + 타임시프트 대응 ── */
    {
      ...
      if (live) {
        ...
        async function liveTickInner() {
          ...
          /* 현재 시각에 프레임이 없다(204) = 세트가 이미 끝났을 가능성 — 세트 종료 후에 접속한
             경우 여기서 영영 대기하게 되므로, 마지막 프레임을 조회해 종료를 확정하고 최종 장면을 표시 */
          if (!st && !sel.e.end && Date.now() - sel.e.start.getTime() > 120000) {
            var wEnd = await jOr(FEED + "/window/" + curId + "?startingTime=" + iso(floor10(endProbeTime(sel.e.start))));
            var lf = wEnd && wEnd.frames && wEnd.frames.length ? wEnd.frames[wEnd.frames.length - 1] : null;
            if (lf && lf.gameState === "finished") {
              sel.e.end = new Date(lf.rfc460Timestamp);
              log(curNum + "세트는 이미 종료 — 최종 상황 표시, 다음 세트 시작 시 자동 전환");
            }
          }
        }
